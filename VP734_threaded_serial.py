    ...
    while event.is_set():
        ...
        if serial.in_waiting:
            ...
            if data_str.endswith(b'\r\n'):
                ...
                # problem here: there's no way to look ahead and verify that the next char is '>'
                # when it spits out the firmware version info the 'V' gets removed from each version #
                # because each version # is separated by ' \r\n', not '\r\n>'
                serial.read()

            response = data_str.strip().decode()
            print("Data received: {}".format(response))
...
def parse(data_str: str):
    ...
    :param data_str: (str) response to be parsed
    ...
    elif 'VTR' in data_str:
        return "<FIRMWARE VERSION {}>".format(data_str.split()[0].strip())
    ...
    if len(data_parts) > 2 and int(data_parts[2]) in functions.keys():
        ...
        if len(data_parts) > 3:
            ...
            if len(data_parts) == 7:
                ...
                return functions[func].format(addr)
